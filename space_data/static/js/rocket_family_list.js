// rocket_family_list.js

document.addEventListener('DOMContentLoaded', () => {
    const table = document.getElementById('rocketFamiliesTable');
    ...
    const sortTable = (columnIndex) => {
        ...
        const numericColumns = ['price', 'launch_thrust', 'payload_mass_leo', 'payload_mass_gto', 'rocket_height', 'fairing_diameter', 'fairing_height', 'stages', 'side_boosters'];
    ...
    document.getElementById('downloadCsvButton').addEventListener('click', () => {
        ...
        const csvContent = rows.map(row => {
            return Array.from(row.cells).map((cell, index) => {
                // Проверяем, содержит ли ячейка ссылку с классом 'wiki-link'
                const link = cell.querySelector('a.wiki-link');
                let text = '';
        ...
        link.href = url;
        link.download = 'rocket_families.csv';
